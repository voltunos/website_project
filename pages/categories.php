...
$stmt = $pdo->prepare("SELECT * FROM categorias WHERE activo = 1");
$stmt->execute();
...
<body>
    ...
    <span class="edit-title">Categorias</span>
     <?php
    if ($error) {
    ...
    <div class="space">
        ...
        foreach($categorias as $categoria) {
            ...
            echo '</form>';
            echo '<form action="deactivate_category.php" method="POST">';
            echo '<input type="hidden" name="id_categoria" value="'.$categoria['id_categoria'].'">';
            echo '<button class="product-edit-deletebutton" type="submit">Desactivar categoria</button>';
            echo '</form>';
            echo '</div>';
            echo '</div>';
        ...
        <div class="product-edit-more">
            ...
            <a href="deactivated_categories.php"><span class="product-edit-additional">Categorias desactivadas</span><img src="../images/deactivated.png" class="icon"></a>
        </div>
    </div>
</body>